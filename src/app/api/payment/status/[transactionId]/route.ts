  ...
  try {
    ...
    const order = await prisma.pendingOrder.findFirst({
      ...
      select: {
        status: true,
        transactionId: true,
        // attendeeId is selected but only ever included in the response
        // below once status is COMPLETED — see note there.
        attendeeId: true,
      },
    });

    if (order) {
      // ✅ Only expose attendeeId once the order is actually completed.
      // Before that, withholding it prevents a mid-poll caller from
      // learning an attendee reference before payment is confirmed.
      return NextResponse.json({
        status: order.status,
        transactionId: order.transactionId,
        ...(order.status === 'COMPLETED' ? { attendeeId: order.attendeeId } : {}),
      });
    }
